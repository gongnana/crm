...
enroll_status_choices = (('singed', '已报名'),
                         ('unregistered', '未报名'),
                         ('studying', '学习中'),
                         ('paid_in_full', '学费已交齐'),
                         )
...
class Customer(models.Model):
    ...
    gender = models.CharField('性别', choices=gender_type, max_length=16, default='male', blank=True, null=True)
    birthday = models.DateField('出生日期', default=None, help_text='格式yyyy-mm-dd', blank=True, null=True)
    phone = models.CharField('手机号', blank=True, null=True, max_length=20)
    source = models.CharField('客户来源', max_length=64, choices=source_type, default='qq')
    introduce_from = models.ForeignKey('self', verbose_name='转介绍自学员', blank=True, null=True, on_delete=models.CASCADE)  # self 指的就是自己这张表, 与models.ForeignKey('Customer', verbose_name='转介绍自学员', blank=True, null=True, on_delete=models.CASCADE)是一样的效果
    course = MultiSelectField('咨询课程', choices=course_choices)  # 多选，存成一个列表的格式，不用多选，也可以用 CharField也存储
    ...
    # 一个客户可以报多个班，报个脱产班，再报周末班等，所以是多对多的关系
    class_list = models.ManyToManyField('ClassList', verbose_name='已报班级', blank=True)

    class Meta:
        ordering = ['id']  # 所有的字段都按照id正向排列
        verbose_name = '客户信息表'  # admin中使用的，以复数的形式展示，加s，客户信息表s
        verbose_name_plural = '客户信息表'  # 不加s
    ...
    def get_classlist(self):
        l = []
        for cls in self.class_list.all():  # 通过self.get_classlist的时候，拿到了所有的班级信息，前端显示的时候用
            l.append(str(cls))
        return mark_safe(','.join(l))  # 纯文本，不用mark_safe也可以
...
class ClassList(models.Model):
    ...
    memo = models.CharField('说明', max_length=100, blank=True, null=True)
...
class Enrollment(models.Model):
    ...
    your_exception = models.TextField('学完想要达成的具体期望', max_length=1024, blank=True, null=True)
    ...
    class Meta:
        unique_together = ('enrollment_class', 'customer')  # 班级和客户联一，一个班级只能有同一个学生
...
class CourseRecord(models.Model):
    ...
    home_title = models.CharField('本节作业标题', max_length=64, blank=True, null=True)
...
class StudyRecord(models.Model):
    ...
    homework = models.FileField('作业文件', blank=True, null=True, default=None)
